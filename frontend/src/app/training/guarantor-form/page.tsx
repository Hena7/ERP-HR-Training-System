...
export default function TrainingGuarantorFormPage() {
  ...
  const [activeTab, setActiveTab] = useState<"guarantors" | "witnesses">(
    "guarantors",
  );
  ...
  const maxRequired =
    selectedContract && selectedContract.totalCost >= 800_000 ? 2 : 1;
  ...
  const handleAddGuarantor = async (e: React.FormEvent) => {
    ...
    if (guarantors.length >= maxRequired) {
      setError(`Maximum ${maxRequired} guarantors allowed for this contract`);
      return;
    }
    setLoading(true);
    setError("");
  ...
  const handleAddWitness = async (e: React.FormEvent) => {
    ...
    if (witnesses.length >= maxRequired) {
      setError(`Maximum ${maxRequired} witnesses allowed for this contract`);
      return;
    }
    setLoading(true);
    setError("");
        ...
        <div className="rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <GroupedTable
              rows={contracts}
              groupBy={(c) =>
                c.trainingTitle || c.trainingType || "General Training"
              }
              rowKey={(c) => c.id}
              ...
              <div className="space-y-6">
                            ...
                            {g.scannedDocument && (
                              <button
                                onClick={() => {
                                  ...
                                  win?.document.write(
                                    `<iframe src="${g.scannedDocument}" width="100%" height="100%" style="border:none;"></iframe>`,
                                  );
                                }}
                ...
                {guarantors.length < maxRequired ? (
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm({ ...form, birthAddress: e.target.value })
                          }
                          className={fieldClass}
                  ...
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-center">
                    <p className="text-sm font-semibold text-emerald-700">
                      Maximum guarantors reached ({maxRequired}/{maxRequired}).
                    </p>
                  </div>
                )}
              </div>
            ...
            {activeTab === "witnesses" && (
              <div className="space-y-6">
                ...
                {witnesses.length < maxRequired ? (
                  ...
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-center">
                    <p className="text-sm font-semibold text-emerald-700">
                      Maximum witnesses reached ({maxRequired}/{maxRequired}).
                    </p>
                  </div>
                )}
              </div>
