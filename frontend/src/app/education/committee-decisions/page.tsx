...
export default function CommitteeDecisionsPage() {
  ...
  const loadData = async () => {
    try {
      const [decRes, scoredRes, reviewRes, allReqRes, oppRes] =
        await Promise.all([
  ...
  const handleBulkReport = async (groupIds: number[]) => {
    ...
    if (
      !confirm(
        `Are you sure you want to forward these ${groupIds.length} candidates to the Director?`,
      )
    )
      return;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium italic">
              Rank candidates per department, vote, and forward to Director
              (requires 4/7 approvals).
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
          <div className="border-b border-gray-50 bg-gray-50/30 px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-[10px] font-bold text-gray-400">
              ...
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-gray-300"></span> Over
                Quota
              </div>
            </div>
          </div>
          ...
          <GroupedTable
            rows={scoredRequests}
            groupBy={(req) =>
              `${req.employeeDepartment || "Unknown"}|||${req.fieldOfStudy || (req as any).educationType || "Unknown"}`
            }
            rowKey={(req) => req.id}
            columns={[
              {
                header: "Rank",
                render: (req) => {
                  // Rank is calculated per group, but GroupedTable doesn't expose index easily.
                  // However, we can sort the rows before passing them.
                  // For now, I'll use a simple indicator.
                  return (
                    <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 text-[10px] font-black text-gray-500">
                      #
                    </span>
                  );
                },
              ...
              {
                header: "Approvals (4/7)",
                render: (req) => {
                  const approvals = req.committeeApprovalCount || 0;
                  const rejections = decisions.filter(
                    (d) => d.requestId === req.id && d.decision === "REJECTED",
                  ).length;
                  return (
                    <div className="flex flex-col items-center gap-1">
                      <span
                        className={`inline-flex items-center justify-center rounded-full px-2.5 py-0.5 text-[10px] font-black ${approvals >= 4 ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-600"}`}
                      >
                        {approvals}/7
                      </span>
                      {rejections > 0 && (
            ...
            renderActions={(req) => {
              const myDecision = decisions.find(
                (d) => d.requestId === req.id && d.decidedBy === user?.username,
              );
              return (
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    onClick={() => toggleDecision(req, "APPROVED")}
                  ...
                  <button
                    onClick={() => toggleDecision(req, "REJECTED")}
            ...
            renderGroupHeader={(groupKey, groupRows, isOpen, toggle) => {
              const [dept, oppStr] = groupKey.split("|||");
              ...
              if (
                opp &&
                (opp as any).departmentQuotas &&
                (opp as any).departmentQuotas[dept]
              ) {
              ...
              const sortedReqs = [...groupRows].sort(
                (a, b) => (b.totalScore || 0) - (a.totalScore || 0),
              );
              const forwardableIds = sortedReqs
                .filter(
                  (r, idx) =>
                    idx < totalQuota && (r.committeeApprovalCount || 0) >= 4,
                )
                .map((r) => r.id);

              return (
                <div
                  className="flex items-center gap-3 bg-gradient-to-r from-blue-900 to-indigo-900 px-6 py-3.5 border-b border-white/10 group cursor-pointer"
                  onClick={toggle}
                >
                  <div className="flex items-center justify-center rounded-md bg-white/10 p-0.5">
                    {isOpen ? (
                  ...
                  <span className="ml-4 text-[10px] font-bold text-blue-300 uppercase tracking-widest">
                    Quota: {totalQuota}{" "}
                    <span className="lowercase text-blue-400">
                      ({candidatesQuota} cand. + {standbyQuota} standby)
                    </span>
                  </span>
                  <div className="ml-auto" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="rounded-lg bg-emerald-500 px-4 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-emerald-600 disabled:opacity-50 transition-all flex items-center gap-2"
                    >
            ...
            emptyMessage="No scored requests available for ranking."
          />
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <p className="text-xl font-black text-blue-700">
                  {selectedScoring.totalScore.toFixed(2)}%
                </p>
              </div>
            </div>
