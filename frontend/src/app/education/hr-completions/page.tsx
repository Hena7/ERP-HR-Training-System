...
export default function HRCompletionsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter completions that were sent to HR
      const allCompletions = compRes.data.content || [];
      const hrCompletions = allCompletions.filter((c: any) => c.sentToHr === true);
      
      setCompletions(hrCompletions);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-green-700 shadow-md">
            <ClipboardCheck className="h-6 w-6 text-white" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">HR Completion Notifications</h1>
            <p className="text-sm text-gray-500 font-medium italic">
              Review and acknowledge education completion records sent from CDC.
            </p>
          </div>
        </div>
